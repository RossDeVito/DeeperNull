""" Custom LibMTL Trainer that allows user to specify device."""
...
class PerformanceMeter(_PerformanceMeter):
	"""Modify performance meter to remove test set evaluation."""

	def __init__(self, task_dict, multi_input):
		super().__init__(task_dict, multi_input)

	def display(self, mode, epoch):
		if epoch is not None:
			if epoch == 0 and self.base_result is None and mode=='val':
				self.base_result = self.results
			if mode == 'train':
				print('Epoch: {:04d} | '.format(epoch), end='')
			if not self.has_val and mode == 'test':
				raise ValueError('Should not use test mode.')
		...
		print(' | ', end='') if mode!='val' else print()


class MTTrainer(nn.Module):
	...
	Args:
		...
		device (str): The device to use, e.g., ``'cpu'`` (default) or ``'cuda:0'``.
	...
	def __init__(self, task_dict, weighting, architecture, encoder_class, decoders, 
				 ...
				 save_path=None, load_path=None, device='cpu',
				 **kwargs):
		super().__init__()
		...
		self.meter = PerformanceMeter(self.task_dict, self.multi_input)
	...
	def _process_data_pred(self, loader):
		try:
			data = next(loader[1])
		except:
			loader[1] = iter(loader[0])
	...
	def _compute_loss(self, preds, gts, task_name=None):
		if not self.multi_input:
			...
			for tn, task in enumerate(self.task_name):
				# If preds[task] is a single value, add a dimension to make it a tensor.
				try:
					...
					print(f"preds[task]: \n\n {preds[task]} \n\n", flush=True)
					print(f"preds[task].device: {preds[task].device}", flush=True)
					print(isinstance(preds[task], torch.Tensor), flush=True)
					print(f"gts[task]: \n\n{gts[task]} \n\n", flush=True)
					print(f"gts[task].device: {gts[task].device}", flush=True)
	...
	def train(
		...
		Args:
			...
			early_stopping_patience (int): The number of epochs to wait for improvement before stopping the training early. \
		...
		for epoch in range(epochs):
			...
			if early_stopping_patience is not None and val_dataloaders is not None:
	...
	def predict(self, pred_dataloaders):
		"""Makes batched predictions using self.model, stacks the outputs
		and returns them as a dict with numpy arrays.

		Args:
			pred_dataloaders (dict or torch.utils.data.DataLoader): The dataloaders used for prediction. \
							If ``multi_input`` is ``True``, it is a dictionary of name-dataloader pairs. \
							Otherwise, it is a single dataloader which returns data in each iteration.

		Returns:
			dict: A dictionary where keys are task names and values are numpy arrays of predictions.
		"""
		
		pred_loader, pred_batch = self._prepare_dataloaders(pred_dataloaders)
		self.model.eval()
		results = defaultdict(list)
		with torch.no_grad():
			if not self.multi_input:
				for _ in range(pred_batch):
					inputs = self._process_data_pred(pred_loader)
					outputs = self.model(inputs)
					outputs = self.process_preds(outputs)
					for task, preds in outputs.items():
						results[task].append(preds.detach().cpu().numpy())
			else:
				for tn, task in enumerate(self.task_name):
					for _ in range(pred_batch[tn]):
						inputs = self._process_data_pred(pred_loader[task])
						outputs = self.model(inputs, task)[task]
						outputs = self.process_preds(outputs)
						results[task].append(outputs.detach().cpu().numpy())
		for task in results:
			results[task] = np.concatenate(results[task], axis=0)
		return results