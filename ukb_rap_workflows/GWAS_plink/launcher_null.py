...
Example usage:
```
python launcher.py -p platelet_count_30080
```
...
WORKFLOW_ID = 'workflow-Gv3JKfQJv7BF53bfP3k7yF22'
...
def parse_args():
	...
	parser.add_argument(
		'--covar-set',
		required=True,
		help='Name of base covariate set file in covar_dir w/o .tsv extension.'
	)
	parser.add_argument(
		'--null-covar-set',
		required=True,
...
def launch_gwas_workflow(
	...
	Args:
		...
		name: Name of the job. Defaults to 'gwas_plink_null'.
	"""
	...
	null_train_pred_link = dxpy.dxlink(
		list(dxpy.find_data_objects(
			name=null_train_pred_file.split('/')[-1],
			folder='/'.join(null_train_pred_file.split('/')[:-1]),
			project=dxpy.PROJECT_CONTEXT_ID
		))[0]['id']
	)
	null_test_val_pred_link = dxpy.dxlink(
		list(dxpy.find_data_objects(
			name=null_test_val_pred_file.split('/')[-1],
			folder='/'.join(null_test_val_pred_file.split('/')[:-1]),
			project=dxpy.PROJECT_CONTEXT_ID
		))[0]['id']
	)
...
if __name__ == '__main__':
	...
	output_dir = f'{args.output_dir}/{args.pheno_name}/{covar_set}_null_{args.null_model}_{args.null_covar_set}'
	print(f'Output directory: {output_dir}')

	# Launch the GWAS workflow
	job_name = f'gwas_plink_{args.pheno_name}_{covar_set}_null_{args.null_model}_{args.null_covar_set}'
