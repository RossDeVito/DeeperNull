"""Script to launch UKB multi-task DeeperNull model fitting workflow.
...
* -p, --pheno (list of str): List of the phenotype names to use as
	input. Corresponds to files in pheno_dir.
...
CPU_WORKFLOW_ID = 'workflow-GyB2kXQJv7B1vxQbzfG6px7Z'
GPU_WORKFLOW_ID = None
...
def parse_args():
	...
	parser.add_argument(
		'-p', '--pheno', nargs='+', required=True,
		help='List of the phenotype file names to use as input. Corresponds to a file in pheno_dir.'
	)
	parser.add_argument(
		'-s', '--save-dir', type=str,
...
def launch_fit(
	...
	instance_type,
	workflow_id,
	name='fit_dn_mt_model'
):
	"""Launch DeeperNull multi-task model fitting workflow.
	...
	Args:
		...
		pheno_file (list of str): Paths to phenotype files.
	...
	workflow_input = {
		...
		f"{prefix}pheno_files": pheno_file_links,
...
if __name__ == '__main__':
	...
	pheno_files = [f'{args.pheno_dir}/{pheno}.pheno' for pheno in args.pheno]
	...
	workflow = launch_fit(
		...
		name=f'{args.model_desc}_mt_{args.covar_set}'
	)
